...
time = []
time1 = []
with open('vector-pop-back.txt', 'r', encoding = 'utf8') as f:
    for line in f:
        time.append(int(line))

with open('subvector_pop_back_time.txt', 'r', encoding = 'utf8') as f:
    for line in f:
...
t1 = np.polyfit(np_size, np_time1,  1)
f1 = np.poly1d(t1)
...
plt.legend()
plt.savefig("График зависимости реализации pop_back для вектора и сабвектора от size", dpi=600)
plt.show()
